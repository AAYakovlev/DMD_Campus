...
-- add guest to person
-- -----------------------
CREATE OR REPLACE FUNCTION public.add_guest_to_person(_guest_id integer, _person_id integer)
 RETURNS BOOL
 LANGUAGE plpgsql
AS $function$
...
BEGIN
 ...
 END IF;
  ...
  INSERT INTO guest_to_person VALUES (_guest_id,_person_id,now(),NULL);
  RETURN TRUE;
END;
$function$
...
BEGIN
  SELECT person.person_id FROM person WHERE person.date_of_birth = _date_of_birth
                                        ...
                                        AND person.gender = _gender
  INTO new_person_id;
  IF (new_person_id IS NOT NULL )
    THEN
      SELECT guest_id FROM guest WHERE person_id = new_person_id INTO new_guest_id;
      ...
      IF (SELECT add_guest_to_person(new_guest_id,_host_person_id) = TRUE)
        ...
        ELSE
          BEGIN
            ...
            DELETE FROM guest WHERE guest_id = new_guest_id;
          END;
          RETURN FALSE;
      END IF;
  END IF;
...
CREATE OR REPLACE FUNCTION public.guest_left_from_person(_guest_id integer, _person_id integer)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
